...
class Window:
    ...
    def exportInsert(self):
        ...
        #if int((self.df.label.max()-self.df.label.min())*3+12)*0.7 < self.sim.vr:
        if 80 < self.sim.vr:
            print("zham")
            self.colorV=(250, 0, 0)
        else:
    ...
    def loop(self, loop=None):
        ...
        # To draw text
        pygame.font.init()
        self.text_font = pygame.font.SysFont('Lucida Console', 32,bold=True)
    ...
    def background(self, r, g, b):
        ...
        self.bg_img = pygame.image.load(r'C:\Users\ZIYAD\Desktop\rev\trafficSimulator\trafffic2.png')
    ...
    def draw_vehicle(self, vehicle, road):
        ...
        self.rotated_box((x, y), (l, h), cos=cos, sin=sin, centered=True,color=self.colorV)
    ...
    def draw_status(self):
        text_fps = self.text_font.render(f'temps={self.sim.t:.5}', False, (0,0,0))
        #text_frc = self.text_font.render(f'frames={self.sim.frame_count}', False, (253,51,36))
        text_vr = self.text_font.render(f'vehicle_rate={self.sim.vr}', False, (0,0,0))
        text_as = self.text_font.render(f'avgSpeed(target)={int((0-self.sim.vr)/2+self.sim.df.label.max())}', False, (0,0,0))
        text_p = self.text_font.render(f'pre_instant={self.sim.prediction}', False, (0,0,250))
        text_pp = self.text_font.render(f'pre_1h+={self.sim.pp}', False, (0,0,250))
        text_h = self.text_font.render(f'heures={self.sim.i}', False, (0,0,0))

        
        self.screen.blit(text_fps, (0, 0))
        #self.screen.blit(text_frc, (250, 0))
        self.screen.blit(text_h, (500, 620))
        self.screen.blit(text_as, (0, 650))
        self.screen.blit(text_p, (500, 650))
        self.screen.blit(text_pp, (1000, 650))
        self.screen.blit(text_vr, (0, 620))
